...
\begin{itemize}
   ...
   \item[] $\rho$ är densiteten, $\unit{kgm^{-3}}$
   \item[] $c_p$ specifika värmekapaciteten, $\unit{J kg^{-1} K^{-1})}$
\end{itemize}
...
Värmeledningsförmågan, eller termisk konduktivitet, är ett materials förmåga att leda värme och mäts i $\unit{W m^{-1} K^{-1})}$. I Schroeder \cite{schroeder00} återfinns en beräkning av luftens värmeledningsfömåga, se ekvation (\ref{eq:natconst:schroeder1}) och (\ref{eq:natconst:schroeder2}), som ger $\unit[0,031]{W m^{-1} K^{-1})}$. Som också beskrivs där är detta tyvärr en ganska approximativ beräkning och den uppmätta värmeledningsförmågan för luft vid rumstemperatur är $\unit[0,024]{W m^{-1} K^{-1})}$.
